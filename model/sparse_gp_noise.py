...
# TODO adapt from Google Research 
# TODO https://github.com/google-research/torchsde
...
class SparseGPNoise(nn.Module):
    def __init__(
        ...
        # intialize such that the output LRD
        self.hurst_net[-2].weight.data.fill_(0.8)
        self.hurst_net[-2].bias.data.fill_(0.8)
    ...
    def generate_noise(self, size):
        '''
        generate_noise: create realizations of mfbm noise 
        size (int): size of batch of generated samples (# of realizations)
        '''
        ...
        for ta, tb in zip(ts[:-1], ts[1:]):
            ...
            sns.lineplot(data=np.cumsum(delta_b.flatten()))#.flatten())
            plt.show()
...
def get_data():
    ...
    alpha = 0.5
    beta = 0.5
    true_wn = SparseGPNoise(t0=t0, t1=t1, dt=1e-2, num_steps=500, num_inducings=50)

    true_wn.precompute(batch_size=batch_size)

    ts, B_h, _ = true_wn.generate_noise(batch_size)
