...
# TODO (option) Apply z-score in a rolling way that does not create lookahead bias 
...
for ts_block in range(wlen, len(time_series2), wlen):  
  ...
  # Offset TODO     
  local_time_series = time_series2[ts_block - wlen: ts_block]     
    
  # Evaluate kernel self similarity matrix 
  kernel = gpytorch.kernels.RBFKernel(lengthscale=10)
  eigenvalues, _ = np.linalg.eig((
      kernel(torch.tensor(local_time_series)).evaluate()).detach().numpy() 
    )  
  
  # Auto-correlation decay
  def get_decaydf(series): 
    decaydf = pd.DataFrame()
    decaydf['series'] = series
    return decaydf
    
  decay_func = lambda S, _range: [ get_decaydf(S)['series'].autocorr(lag=Lag) for Lag in range(_range) ]
  ...
  previous_timeseries = local_time_series
  
fig, ax = plt.subplots(2)
axx = ax[0].twinx()
sns.lineplot(data=wasserstein_distance_over_time, ax=ax[1], 
              label='wasserstein_dist_over_time', linestyle='--', color='red', alpha=0.45)
sns.lineplot(data=np.cumsum(time_series2), ax=ax[0])
sns.lineplot(data=previous_eigenvalues, ax=axx)
plt.legend()
plt.show()
